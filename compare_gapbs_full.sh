...
# Logs are *.full.rN.txt and the CSV is *.compare.full.csv, so these runs do
# not overwrite or continue the kernel-only results.
# Graphs are shared serialized .sg/.wsg files under $RESULTS_DIR/graphs/.
#
# Usage:
#   ./compare_gapbs_full.sh
#   GRAPHS="kron22" NTHREADS=16 RUNS=5 ./compare_gapbs_full.sh
#   GRAPHS="kron10" KERNELS="bfs pr" RUNS=1 WARMUP=0 ./compare_gapbs_full.sh
#   START=6 RUNS=10 WARMUP=0 ./compare_gapbs_full.sh   # request at least r6; script auto-skips old runs
set -euo pipefail
...
max_existing_run() {
  ...
  shopt -s nullglob
  for tag in "${TAG_ARR[@]}"; do
    for path in "$app_dir/${kernel}.${graph}.t${NTHREADS}.${tag}.full.r"*.txt; do
      base="${path##*/}"
      if [[ "$base" =~ \.full\.r([0-9]+)\.txt$ ]]; then
        run="${BASH_REMATCH[1]}"
...
max_existing_warmup() {
  ...
  shopt -s nullglob
  for tag in "${TAG_ARR[@]}"; do
    for path in "$app_dir/${kernel}.${graph}.t${NTHREADS}.${tag}.full.warmup"*.txt; do
      base="${path##*/}"
      if [[ "$base" =~ \.full\.warmup([0-9]+)\.txt$ ]]; then
        run="${BASH_REMATCH[1]}"
...
for graph in $GRAPHS; do
  local_csv="${COMPARE_CSV:-$RESULTS_DIR/gapbs.${graph}.t${NTHREADS}.compare.full.csv}"
  if [[ ! -f "$local_csv" ]]; then
  ...
  for kernel in $KERNELS; do
      ...
      for ((i = warmup_start; i < warmup_start + WARMUP; i++)); do
        for idx in "${!CONFIG_ARR[@]}"; do
          run_one "${CONFIG_ARR[$idx]}" "$kernel" "$graph" "${TAG_ARR[$idx]}" \
            "$app_dir/${kernel}.${graph}.t${NTHREADS}.${TAG_ARR[$idx]}.full.warmup${i}.txt" \
            "warmup$i" >/dev/null
        done
      done
    fi
    for ((i = run_start; i <= run_end; i++)); do
      for idx in "${!CONFIG_ARR[@]}"; do
        cfg="${CONFIG_ARR[$idx]}"
        name="${TAG_ARR[$idx]}"
        log="$app_dir/${kernel}.${graph}.t${NTHREADS}.${name}.full.r${i}.txt"
