...
#
# Times are each GAPBS portion, not a single kernel average or wall-clock total.
# For the serialized graphs this script loads, "read" is file load plus CSR
# index construction (GAPBS "Read Time"). "trial" is one kernel execution
# (GAPBS "Trial Time"), numbered from 1 in order. "build" and "generate"
# appear only if the binary prints them. "wall" is `time -p` real seconds.
# Logs are *.breakdown.rN.txt and the CSV is *.compare.breakdown.csv.
# Graphs are shared serialized .sg/.wsg files under $RESULTS_DIR/graphs/.
#
# Usage:
#   ./compare_gapbs_breakdown.sh
#   GRAPHS="kron22" NTHREADS=16 RUNS=5 ./compare_gapbs_breakdown.sh
#   GRAPHS="kron10" KERNELS="bfs pr" RUNS=1 WARMUP=0 ./compare_gapbs_breakdown.sh
#   START=6 RUNS=10 WARMUP=0 ./compare_gapbs_breakdown.sh   # request at least r6; script auto-skips old runs
set -euo pipefail
...
record_portions() {
  local config="$1" kernel="$2" graph="$3" run="$4" log="$5" status="$6"
  awk -v config="$config" -v kernel="$kernel" -v graph="$graph" \
      -v threads="$NTHREADS" -v run="$run" -v status="$status" '
    function row(portion, idx, secs) {
      print config "," kernel "," graph "," threads "," run "," portion "," idx "," secs "," status
    }
    /^Read Time:/ { row("read", "", $NF); n++ }
    /^Build Time:/ { row("build", "", $NF); n++ }
    /^Generate Time:/ { row("generate", "", $NF); n++ }
    /^Relabel:/ { row("relabel", "", $NF); n++ }
    /^Trial Time:/ { trial++; row("trial", trial, $NF); n++ }
    /^Verification Time:/ { verify++; row("verify", verify, $NF); n++ }
    /^real[[:space:]]/ { row("wall", "", $2); n++ }
    END { if (n == 0) row("none", "", "") }
  ' "$log"
}
...
crash_note() {
  if grep -Eiq 'Segmentation fault|Aborted|core dumped|bus error|Illegal instruction' "$1"; then
    echo crash
  elif ! grep -Eq '^Read Time:|^Trial Time:|^real[[:space:]]' "$1"; then
    echo no_time
  else
    echo ok
  fi
...
run_one() {
  ...
  record_portions "$config" "$kernel" "$graph" "$run" "$log" "$status"
}
...
max_existing_run() {
  ...
  shopt -s nullglob
  for tag in "${TAG_ARR[@]}"; do
    for path in "$app_dir/${kernel}.${graph}.t${NTHREADS}.${tag}.breakdown.r"*.txt; do
      base="${path##*/}"
      if [[ "$base" =~ \.breakdown\.r([0-9]+)\.txt$ ]]; then
        run="${BASH_REMATCH[1]}"
...
max_existing_warmup() {
  ...
  shopt -s nullglob
  for tag in "${TAG_ARR[@]}"; do
    for path in "$app_dir/${kernel}.${graph}.t${NTHREADS}.${tag}.breakdown.warmup"*.txt; do
      base="${path##*/}"
      if [[ "$base" =~ \.breakdown\.warmup([0-9]+)\.txt$ ]]; then
        run="${BASH_REMATCH[1]}"
...
for graph in $GRAPHS; do
  local_csv="${COMPARE_CSV:-$RESULTS_DIR/gapbs.${graph}.t${NTHREADS}.compare.breakdown.csv}"
  if [[ ! -f "$local_csv" ]]; then
    echo "config,app,input,threads,run,portion,index,time_s,status" >"$local_csv"
  fi
  for kernel in $KERNELS; do
      ...
      for ((i = warmup_start; i < warmup_start + WARMUP; i++)); do
        for idx in "${!CONFIG_ARR[@]}"; do
          run_one "${CONFIG_ARR[$idx]}" "$kernel" "$graph" "${TAG_ARR[$idx]}" \
            "$app_dir/${kernel}.${graph}.t${NTHREADS}.${TAG_ARR[$idx]}.breakdown.warmup${i}.txt" \
            "warmup$i" >/dev/null
        done
      done
    fi
    for ((i = run_start; i <= run_end; i++)); do
      for idx in "${!CONFIG_ARR[@]}"; do
        cfg="${CONFIG_ARR[$idx]}"
        name="${TAG_ARR[$idx]}"
        log="$app_dir/${kernel}.${graph}.t${NTHREADS}.${name}.breakdown.r${i}.txt"
