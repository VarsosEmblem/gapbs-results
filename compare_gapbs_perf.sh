...
#   cycles                         cpu cycles
#   cycle_activity.stalls_l2_miss  execution stalls while an L2 miss is outstanding
#   MPKI                           1000 * l2_rqsts.miss / instructions
# Logs are *.perf.rN.txt and the CSV is *.compare.perf.csv.
# Override the event list with PERF_EVENTS if a name is unsupported.
# Graphs are shared serialized .sg/.wsg files under $RESULTS_DIR/graphs/.
#
# Usage:
#   ./compare_gapbs_perf.sh
#   GRAPHS="kron24" NTHREADS=64 CONFIGS="clang-plain clang-plainje clang-chonk" TAGS="plain plainje chonk" RUNS=5 ./compare_gapbs_perf.sh
#   START=6 RUNS=10 WARMUP=0 ./compare_gapbs_perf.sh
set -euo pipefail
...
PERF_EVENTS="${PERF_EVENTS:-cycles,instructions,cycle_activity.stalls_l2_miss,l2_rqsts.miss}"
...
parse_perf() {
  awk -F, '
    function clean(s) {
      gsub(/^[ \t]+|[ \t]+$/, "", s)
      if (s ~ /not supported|not counted/) return ""
      return s
    }
    {
      ev = $3
      gsub(/^[ \t]+|[ \t]+$/, "", ev)
      val = clean($1)
      if (ev == "cycles" || ev == "cpu-cycles" || ev ~ /(^|\/)cycles\/?$/)
        cycles = val
      else if (ev ~ /stalls_l2_miss/)
        stalls = val
      else if (ev ~ /l2_rqsts\.miss/)
        misses = val
      else if (ev == "instructions" || ev == "cpu-instructions" || ev ~ /(^|\/)instructions\/?$/)
        insns = val
    }
    END {
      mpki = ""
      if (insns != "" && misses != "" && insns + 0 > 0)
        mpki = sprintf("%.4f", 1000 * misses / insns)
      printf "%s,%s,%s\n", cycles, stalls, mpki
    }
  ' "$1"
}
...
crash_note() {
  local parsed cycles stalls mpki
  if grep -Eiq 'Segmentation fault|Aborted|core dumped|bus error|Illegal instruction' "$1"; then
    echo crash
    return
  fi
  parsed=$(parse_perf "$1")
  IFS=, read -r cycles stalls mpki <<<"$parsed"
  if [[ -z "$cycles" || -z "$stalls" || -z "$mpki" ]]; then
    echo no_counter
  else
    echo ok
  fi
...
run_one() {
  ...
  echo "$config,$kernel,$graph,$NTHREADS,$run,$counters,$status"
}
...
max_existing_run() {
  ...
  shopt -s nullglob
  for tag in "${TAG_ARR[@]}"; do
    for path in "$app_dir/${kernel}.${graph}.t${NTHREADS}.${tag}.perf.r"*.txt; do
      base="${path##*/}"
      if [[ "$base" =~ \.perf\.r([0-9]+)\.txt$ ]]; then
        run="${BASH_REMATCH[1]}"
...
max_existing_warmup() {
  ...
  shopt -s nullglob
  for tag in "${TAG_ARR[@]}"; do
    for path in "$app_dir/${kernel}.${graph}.t${NTHREADS}.${tag}.perf.warmup"*.txt; do
      base="${path##*/}"
      if [[ "$base" =~ \.perf\.warmup([0-9]+)\.txt$ ]]; then
        run="${BASH_REMATCH[1]}"
...
for graph in $GRAPHS; do
  local_csv="${COMPARE_CSV:-$RESULTS_DIR/gapbs.${graph}.t${NTHREADS}.compare.perf.csv}"
  if [[ ! -f "$local_csv" ]]; then
    echo "config,app,input,threads,run,cycles,l2_miss_stall_cycles,mpki,status" >"$local_csv"
  fi
  for kernel in $KERNELS; do
      ...
      for ((i = warmup_start; i < warmup_start + WARMUP; i++)); do
        for idx in "${!CONFIG_ARR[@]}"; do
          run_one "${CONFIG_ARR[$idx]}" "$kernel" "$graph" "${TAG_ARR[$idx]}" \
            "$app_dir/${kernel}.${graph}.t${NTHREADS}.${TAG_ARR[$idx]}.perf.warmup${i}.txt" \
            "warmup$i" >/dev/null
        done
      done
    fi
    for ((i = run_start; i <= run_end; i++)); do
      for idx in "${!CONFIG_ARR[@]}"; do
        cfg="${CONFIG_ARR[$idx]}"
        name="${TAG_ARR[$idx]}"
        log="$app_dir/${kernel}.${graph}.t${NTHREADS}.${name}.perf.r${i}.txt"
